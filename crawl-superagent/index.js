...
superagent
    ...
    .end(function (err, res) {
        ...
        // 并发遍历heroes对象
        async.mapLimit(heroes, 2, function (hero, callback) {
            // 对每个角色对象的处理逻辑
            var heroId = hero[0]  // // 获取角色数据第一位的数据，即：角色id
            fetchInfo(heroId, callback)   // 根据觉得id获取详情的函数
        }, function (err, result) {
...
var fetchInfo = function (heroId, callback) {
    ...
    superagent
        ...
        .end(function (err, res) {
            ...
            concurrencyCount--
            callback(null, heroId)  // 不传递callback()这个就只执行一次
        })
}